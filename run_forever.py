...
One betting session:
    ...
    5. Optional forced accumulator:  --acca 3   (2-4 legs) builds one
       multi-leg slip from the cleanest qualifying picks for bigger
       combined odds, floors intact.
...
Usage:
    ...
    python run_daily.py --session morning --acca 4
"""
...
# Forced-acca floors (intentionally the same quality bar as normal accas).
...
def _build_forced_acca(
    ...
    Greedy by model probability (certainty first), one leg per match,
    floors: leg prob >= 0.55, leg odds <= 2.50, combined odds <= 6.00.
    The true combined probability is printed — always.
    """
    pool = [
        o for o in opportunities
